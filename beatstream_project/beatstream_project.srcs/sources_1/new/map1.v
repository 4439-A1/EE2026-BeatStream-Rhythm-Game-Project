...
integer m;
reg [78:0] moving_arrow = 0;    // Can now handle up to 79 arrows
reg [6:0] y_var [0:78];         // Array for y-positions of up to 79 arrows (7-bit for y values 0-63)
reg y_missing [0:78];         // Array for y-positions of up to 79 arrows (7-bit for y values 0-63)
wire [78:0] missed;
        ...
        for (j = 0; j < 34; j = j + 1) begin
            if (moving_arrow[j]) begin
                if (y_var[j] < 63-5) begin  // Increment y_var until it reaches 63
                    y_var[j] <= y_var[j] + 1;  // Move arrow down
                end
                else begin
            ...
            if(!moving_arrow[j] && missed[j] && y_var[j] == 63-5)  begin
                if (y_missing[j] < 10) begin  // Increment y_var until it reaches 63
                    y_missing[j] <= y_missing[j] + 1;  // Move arrow down
                end
            end
        end
    end
...
always @(posedge slow_clock_20hz) begin
   ...
   if (set && !btnc_prev && miss && health_level >= 10) begin
